#!/usr/bin/env python3
"""Update the existing O008 Figshare metadata/link item for Chapter 11."""
...
def description(
    ...
    return (
        ...
        "Checkpoint ini menambahkan Bab 11, <em>Teorema Gelfand–Naimark</em>. "
        "Bab 12–17, "
        ...
        f"<code>{pdf['sha256']}</code>). PDF 164 halaman ini searchable dan "
...
def project_stored_bytes(
    session: requests.Session, items: list[dict[str, Any]]
) -> int:
    """Count only bytes actually stored by Figshare, not external link targets."""
    total = 0
    for item in items:
        article_id = int(item["id"])
        article_files = checked(
            session.get(
                f"{BASE}/account/articles/{article_id}/files", timeout=30
            ),
            {200},
        ).json()
        total += sum(
...
def main() -> None:
    ...
    project_bytes = project_stored_bytes(session, project_items)
    if project_bytes >= 20_000_000_000:
        raise RuntimeError("Figshare project has reached the 20,000,000,000-byte cap")
            ...
            json.dumps(
                {
                    ...
                    "project_stored_bytes": project_bytes,
                    "project_storage_cap_bytes": 20_000_000_000,
                },
                ensure_ascii=False,
                indent=2,
                sort_keys=True,
    ...
    payload = {
        "title": (
            ...
            "Edisi Bahasa Indonesia (Bab 1–11 dari 17)"
        ),
    ...
    result = {
        ...
        "project": {
            ...
            "stored_bytes": project_bytes,
            "storage_cap_bytes": 20_000_000_000,
            "storage_cap_pass": project_bytes < 20_000_000_000,
        },
        "collection": {
            "id": COLLECTION_ID,
