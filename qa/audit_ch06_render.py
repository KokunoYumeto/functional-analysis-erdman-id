#!/usr/bin/env python3
"""Inventory and contact-sheet the frozen Chapter 1--6 PDF render."""
    ...
    for batch_index, start in enumerate(range(0, len(pages), batch_size), 1):
        ...
        for position, page_path in enumerate(batch):
            ...
            with Image.open(page_path) as page_image:
                ...
                thumb = ImageOps.contain(page_image, THUMB_SIZE)
            paste_x = x + (cell_width - thumb.width) // 2
            paste_y = y + (THUMB_SIZE[1] - thumb.height) // 2
            sheet.paste(thumb, (paste_x, paste_y))
            draw.text((x + 4, y + THUMB_SIZE[1] + 6), f"Page {page_number(page_path)}", fill="black")
...
def main() -> None:
    ...
    manifest = {
        ...
        "source_pdf": "qa/build-through-ch06-a/functional-analysis-id-through-ch06.pdf",
        "source_pdf_bytes": (ROOT / "qa" / "build-through-ch06-a" / "functional-analysis-id-through-ch06.pdf").stat().st_size,
        "source_pdf_sha256": sha256(ROOT / "qa" / "build-through-ch06-a" / "functional-analysis-id-through-ch06.pdf"),
