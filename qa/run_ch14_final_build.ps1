...
$out = Join-Path $lane 'qa\build-through-ch14-final'
$tex = 'functional-analysis-id-through-ch14.tex'
$stem = 'functional-analysis-id-through-ch14'
$pdf = Join-Path $out ($stem + '.pdf')
$pass1Pdf = Join-Path $lane 'tmp\pdfs\ch14-final-pass1.pdf'
$pass1Log = Join-Path $lane 'qa\CH14_FINAL_BUILD_PASS1_LOG.txt'
$pass2Log = Join-Path $lane 'qa\CH14_FINAL_BUILD_PASS2_LOG.txt'
$resultPath = Join-Path $lane 'qa\CH14_FINAL_BUILD_RESULT.json'
$reader = Join-Path $lane 'output\pdf\analisis-fungsional-dan-aljabar-operator-id-bab-1-14.pdf'
...
if ($masterHash -cne 'f04180a796707c6cb0c5f74082a8b4c25721d20ff3ea9235819939b11e1e50c9') {
    throw 'Chapter 14 cumulative master identity differs'
}
$chapterHash = (Get-FileHash -LiteralPath (Join-Path $src 'multiplier_algebras-id.tex') -Algorithm SHA256).Hash.ToLowerInvariant()
if ($chapterHash -cne '2688ec9c2370371060aada680f5f95e9511ecb61cb99c2a126385f525a3c9142') {
    throw 'Chapter 14 target identity differs'
}
...
$result = [ordered]@{
    schema_version = 'o008.ch14-final-build.v1'
