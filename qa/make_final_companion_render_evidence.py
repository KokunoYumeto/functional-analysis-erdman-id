#!/usr/bin/env python3
"""Inventory all final-reader PNGs and create all-page contact sheets."""
...
RENDER = ROOT / "tmp" / "pdfs" / "final-companion"
PAGES = RENDER / "pages"
SHEETS = ROOT / "qa" / "final-companion-contact-sheets"
MANIFEST = ROOT / "provenance" / "FINAL_COMPANION_RENDER_MANIFEST.csv"
SUMMARY = ROOT / "qa" / "FINAL_COMPANION_RENDER_AUDIT.json"
...
def main() -> None:
    ...
    expected_pdf = result["pdf"]
    if PDF.stat().st_size != int(expected_pdf["bytes"]) or sha256(PDF) != expected_pdf["sha256"]:
    ...
    if any(path.exists() for path in (SHEETS, MANIFEST, SUMMARY)):
        raise SystemExit("fresh render evidence targets required")
    SHEETS.mkdir()
    ...
    for number, path in enumerate(pages, 1):
        with Image.open(path) as image:
            ...
            if bbox is None:
                ...
                edge_regions = ((0, 0, image.width, 5), (0, image.height - 5, image.width, image.height), (0, 5, 5, image.height - 5), (image.width - 5, 5, image.width, image.height - 5))
                edge_ink = sum(edge_mask.crop(region).histogram()[255] for region in edge_regions)
                if edge_ink:
                    edge_ink_pages.append(number)
            rows.append({
                "page": number, "file": path.name, "bytes": path.stat().st_size,
                "sha256": sha256(path), "width_px": image.width, "height_px": image.height,
                "ink_left_px": left, "ink_top_px": top, "ink_right_px": right,
                "ink_bottom_px": bottom, "outer_5px_ink": edge_ink,
            })
    ...
    per_sheet, thumb_size, cell_size = 12, (234, 303), (254, 337)
    for start in range(0, len(pages), per_sheet):
        ...
        first, last = start + 1, start + len(selected)
        path = SHEETS / f"contact-{first:03d}-{last:03d}.png"
        sheet.save(path, format="PNG", optimize=True)
        sheet_records.append({"file": path.name, "pages": [first, last], "bytes": path.stat().st_size, "sha256": sha256(path)})

    nonblank = [row for row in rows if row["ink_left_px"] is not None]
    summary = {
        "schema_version": "o008.final-companion-render-audit.v1",
        "pdf": {"path": PDF.relative_to(ROOT).as_posix(), "bytes": PDF.stat().st_size, "sha256": sha256(PDF)},
        "page_count": len(rows), "render_dimensions_px": list(EXPECTED_SIZE),
        "aggregate_png_bytes": sum(int(row["bytes"]) for row in rows),
        "blank_pages": blank_pages, "outer_5px_ink_pages": edge_ink_pages,
        ...
        "manifest": {"path": MANIFEST.relative_to(ROOT).as_posix(), "bytes": MANIFEST.stat().st_size, "sha256": sha256(MANIFEST)},
