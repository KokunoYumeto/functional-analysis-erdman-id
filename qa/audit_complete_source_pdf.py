#!/usr/bin/env python3
"""Bounded navigation, font, action, and security audit for the complete source reader."""
...
RESULT = ROOT / "qa" / "COMPLETE_SOURCE_PDF_SECURITY_NAVIGATION_AUDIT.json"
EXPECTED_BYTES = 2_480_109
EXPECTED_SHA256 = "efa2358b3c3e6e8c47e0caee9a02f6afe78d15bea1b0f1822ea8449d801b2b10"
...
def main() -> None:
    if PDF.stat().st_size != EXPECTED_BYTES or sha256(PDF) != EXPECTED_SHA256:
        raise SystemExit("complete-source PDF identity differs")
    ...
    fonts = font_audit()
    result = {
        "schema_version": "o008.complete-source-pdf-security-navigation-audit.v1",
        "pdf": {
    ...
    failures = []
    if len(reader.pages) != 238 or reader.is_encrypted or unresolved:
    ...
    if any(
        (
            result["acroform"],
            result["embedded_files_name_tree"],
            result["javascript_name_tree"],
            file_attachments,
            rich_media,
            result["launch_actions"],
            result["javascript_actions"],
        )
    ):
    ...
    result["accessibility_limitation"] = (
        "PDF is honestly untagged; the required semantic accessible HTML remains a separate production surface."
    )
