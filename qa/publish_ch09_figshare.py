#!/usr/bin/env python3
"""Update the existing O008 Figshare metadata/link item for Chapter 9."""
...
PDF_URL = f"https://zenodo.org/records/{ZENODO_ID}/files/{PDF_NAME}"
ZIP_URL = f"https://zenodo.org/records/{ZENODO_ID}/files/{ZIP_NAME}"
SUMS_URL = f"https://zenodo.org/records/{ZENODO_ID}/files/{SUMS_NAME}"
...
def description() -> str:
    return (
        ...
        "Checkpoint ini menambahkan Bab 9, <em>Ruang Vektor Topologis</em>. "
        "Bab 10–17, HTML semantik/aksesibel, lapisan solusi O001, dan jembatan "
        ...
        "(1,686,477 byte; SHA-256 "
        "<code>99215ff5987f720600152da902cec8f521329692931a0dbf89e82ef2d4ff6076</code>). "
        "PDF 140 halaman ini searchable dan navigable, tetapi belum bertag.</p>"
        f"<p><strong>Sumber/backend:</strong> <a href=\"{ZIP_URL}\">{ZIP_NAME}</a> "
        "(1,622,383 byte; SHA-256 "
        "<code>b526676d95275ecc22c6e9b3d2373c21472477509de3719d3fb546ee442fa921</code>). "
        f"<a href=\"{SUMS_URL}\">{SUMS_NAME}</a> (253 byte; SHA-256 "
        "<code>47d78cf87c246a3117fb6098b930fe66470cc5a7db485e85e067bb312d4d6bd8</code>).</p>"
        "<p>Zenodo: <a href=\"https://doi.org/10.5281/zenodo.22061720\">"
        "10.5281/zenodo.22061720</a>. GitHub: "
...
def main() -> None:
    ...
    private_collection_items = checked(
        session.get(f"{collection_url}/articles", params={"page_size": 1000}, timeout=30),
        {200},
    ).json()
    ...
    payload = {
        "title": (
            ...
            "Edisi Bahasa Indonesia (Bab 1–9 dari 17)"
        ),
        ...
        "references": [
            "https://doi.org/10.5281/zenodo.22061720",
            "https://zenodo.org/records/22061720",
            "https://web.pdx.edu/~erdman/",
            f"https://zenodo.org/api/records/{ZENODO_ID}/files/{PDF_NAME}/content",
            f"https://zenodo.org/api/records/{ZENODO_ID}/files/{ZIP_NAME}/content",
            f"https://zenodo.org/api/records/{ZENODO_ID}/files/{SUMS_NAME}/content",
    ...
    linked = checked(
        session.post(f"{article_url}/files", json={"link": PDF_URL}, timeout=60), {201}
    )
    ...
    public_article = checked(session.get(f"{BASE}/articles/{ARTICLE_ID}", timeout=30), {200}).json()
    if public_article.get("version") != 3:
        raise RuntimeError(f"unexpected public article version: {public_article.get('version')}")

    private_ids = article_ids(private_collection_items)
    if ARTICLE_ID not in private_ids:
        checked(
            session.post(
                f"{collection_url}/articles", json={"articles": [ARTICLE_ID]}, timeout=60
            ),
            {201},
        )
    checked(session.post(f"{collection_url}/publish", timeout=120), {201})
    collection = checked(session.get(f"{BASE}/collections/{COLLECTION_ID}", timeout=30), {200}).json()
    ...
    result = {
        ...
        "article": {
            ...
            "url": public_article["url_private_api"].replace(
                "api.figshare.com/v2/account/articles", "figshare.com/articles/online_resource"
            )
