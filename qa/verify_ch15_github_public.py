#!/usr/bin/env python3
"""Anonymously verify the Chapter 15 GitHub commit and all changed bytes."""
...
def main() -> None:
    ...
    if len(paths) != 42 or len(paths) != len(set(paths)):
        raise SystemExit(f"unexpected changed-path inventory: {len(paths)}")
    ...
    if mismatches or {item["path"] for item in representative} != REPRESENTATIVE:
        raise RuntimeError(f"public byte readback mismatch: {mismatches}")

    result = {
        "schema_version": "1.0.0",
        "receipt_id": "FAOA-2015-CH15-GITHUB-PUBLICATION",
