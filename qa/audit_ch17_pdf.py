#!/usr/bin/env python3
"""Bounded navigation, structure, action, and security audit for Chapters 1--17."""
...
RESULT = ROOT / "qa" / "CH17_PDF_SECURITY_NAVIGATION_AUDIT.json"
EXPECTED_BYTES = 2_432_395
EXPECTED_SHA256 = "22fda5f25205f2a442c2b907db015fb4c93cb46cfcba6a1fa8814449469073f1"
...
def main() -> None:
    if PDF.stat().st_size != EXPECTED_BYTES or sha256(PDF) != EXPECTED_SHA256:
        raise SystemExit("Chapter 17 PDF identity differs")
    ...
    result = {
        "schema_version": "o008.ch17-pdf-security-navigation-audit.v1",
        ...
        "struct_tree_root": "/StructTreeRoot" in root, "mark_info": "/MarkInfo" in root,
        "outline_entries": outline_count(reader.outline), "named_destinations": len(named),
        "internal_links": internal_links, "unresolved_internal_links": unresolved,
        "unique_uri_targets": sorted(uri_targets),
        "annotation_subtypes": dict(sorted(annotation_subtypes.items())),
        "action_types": dict(sorted(action_types.items())), "open_action": open_action_kind,
        "acroform": "/AcroForm" in root, "embedded_files_name_tree": "/EmbeddedFiles" in names,
        "javascript_name_tree": "/JavaScript" in names,
        "file_attachment_annotations": file_attachments, "rich_media_annotations": rich_media,
        "launch_actions": action_types.get("/Launch", 0),
        "javascript_actions": action_types.get("/JavaScript", 0),
    }
    failures = []
    if len(reader.pages) != 232 or reader.is_encrypted or unresolved:
        failures.append("page, encryption, or destination failure")
    if result["catalog_language"] != "id-ID":
        failures.append("catalog language")
    if any((result["acroform"], result["embedded_files_name_tree"],
            result["javascript_name_tree"], file_attachments, rich_media,
            result["launch_actions"], result["javascript_actions"])):
