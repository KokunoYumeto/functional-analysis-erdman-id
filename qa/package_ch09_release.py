#!/usr/bin/env python3
"""Build and verify the deterministic Chapter 9 Zenodo source/backend bundle."""
...
def main() -> None:
    ...
    metadata = json.dumps(
        {
            ...
            "release": RELEASE,
            "scope": "in progress; Chapters 1--9 of 17",
        ...
        for info in infos:
            with zf.open(info) as stream:
                while stream.read(1024 * 1024):
                    pass
        forbidden_names = [name for name in names if any(name.endswith(x) for x in FORBIDDEN)]
        if forbidden_names:
    ...
    result = {
        ...
        "zip": {"path": str(zip_path), "bytes": len(zip_data), "sha256": sha256(zip_data)},
        "pdf": {"path": str(pdf_path), "bytes": len(pdf_data), "sha256": sha256(pdf_data)},
        "sums": {
            "path": str(sums_path),
            "bytes": len(sums),
            "sha256": sha256(sums),
