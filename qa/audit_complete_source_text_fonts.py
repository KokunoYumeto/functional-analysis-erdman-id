#!/usr/bin/env python3
"""Audit complete-source reader text extraction and Poppler font inventory."""
...
AUDIT_DIR = ROOT / "qa" / "render-complete-source-final"
PDF = ROOT / "output" / "pdf" / "analisis-fungsional-dan-aljabar-operator-id-teks-sumber-lengkap.pdf"
TEXT = AUDIT_DIR / "functional-analysis-id-complete-source-layout.txt"
FONTS = AUDIT_DIR / "functional-analysis-id-complete-source-fonts.txt"
INFO = AUDIT_DIR / "functional-analysis-id-complete-source-pdfinfo.txt"
RESULT = ROOT / "qa" / "COMPLETE_SOURCE_TEXT_FONT_AUDIT.json"
EXPECTED_PAGES = 238
EXPECTED_PDF_BYTES = 2_480_109
EXPECTED_PDF_SHA256 = "efa2358b3c3e6e8c47e0caee9a02f6afe78d15bea1b0f1822ea8449d801b2b10"
...
def main() -> None:
    if PDF.stat().st_size != EXPECTED_PDF_BYTES or sha256(PDF) != EXPECTED_PDF_SHA256:
        raise SystemExit("complete-source PDF identity does not match the frozen audit target")
    subprocess.run(
    ...
    result = {
        "schema_version": "o008.complete-source-text-font-audit.v1",
        "pdf": {
        ...
        "local_path_hits": {
            p: len(re.findall(p, text, flags=re.IGNORECASE))
