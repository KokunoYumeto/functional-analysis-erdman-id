...
$out = Join-Path $lane 'qa\build-through-ch17-final'
$tex = 'functional-analysis-id-through-ch17.tex'
$stem = 'functional-analysis-id-through-ch17'
$pdf = Join-Path $out ($stem + '.pdf')
$pass1Pdf = Join-Path $lane 'tmp\pdfs\ch17-final-pass1.pdf'
$pass1Log = Join-Path $lane 'qa\CH17_FINAL_BUILD_PASS1_LOG.txt'
$pass2Log = Join-Path $lane 'qa\CH17_FINAL_BUILD_PASS2_LOG.txt'
$resultPath = Join-Path $lane 'qa\CH17_FINAL_BUILD_RESULT.json'
$reader = Join-Path $lane 'output\pdf\analisis-fungsional-dan-aljabar-operator-id-bab-1-17.pdf'
...
if ($masterHash -cne '51b3f4d790e3d09ac6ac4c160284510e4827140ecaa5a961a46c880ae5c8bb39') {
    throw 'Chapter 17 cumulative master identity differs'
}
$chapterHash = (Get-FileHash -LiteralPath (Join-Path $src 'K0_functor-id.tex') -Algorithm SHA256).Hash.ToLowerInvariant()
if ($chapterHash -cne '061ffd28907e2251fc8b01077888de3e11b3bce67fec1ba52b080c924a241059') {
    throw 'Chapter 17 target identity differs'
}
...
if ($actualPoints.Count -lt $expectedInheritedPoints.Count) {
    throw "Inherited overfull-box witnesses are missing: $($actualPoints.Count)"
}
...
$result = [ordered]@{
    schema_version = 'o008.ch17-final-build.v1'
    ...
    chapter_17_overfull_boxes = $chapterOverfullCount
