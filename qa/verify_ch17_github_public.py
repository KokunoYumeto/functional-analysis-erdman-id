#!/usr/bin/env python3
"""Anonymously verify the Chapter 17 GitHub commit and all changed bytes."""
...
BRANCH = "main"
RECEIPT = ROOT / "provenance" / "GITHUB_PUBLICATION_RECEIPT_CH17.json"
REPRESENTATIVE = {
    "README.md", "source/id-ID/K0_functor-id.tex",
    "source/id-ID/functional-analysis-id-through-ch17.tex",
    "output/pdf/analisis-fungsional-dan-aljabar-operator-id-bab-1-17.pdf",
    "provenance/CH17_BUILD_AND_QA_RECEIPT.md", "backend/BACKEND_MANIFEST.csv",
}
...
def main() -> None:
    ...
    paths = git_text("diff-tree", "--no-commit-id", "--name-only", "-r", f"{commit}^", commit).splitlines()
    ...
    ref = checked_json(session, f"https://api.github.com/repos/{OWNER}/{REPOSITORY}/git/ref/heads/{BRANCH}")
    remote_head = str(dict(ref.get("object", {})).get("sha", ""))
    commit_payload = checked_json(session, f"https://api.github.com/repos/{OWNER}/{REPOSITORY}/git/commits/{commit}")
    ...
    for path in paths:
        ...
        if path in REPRESENTATIVE:
            representative.append({"path": path, "bytes": len(public),
                                   "sha256": sha256(public), "match": True})

    result = {
        "schema_version": "1.0.0", "receipt_id": "FAOA-2015-CH17-GITHUB-PUBLICATION",
        "verified_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "repository": f"https://github.com/{OWNER}/{REPOSITORY}", "branch": BRANCH,
        "commit": commit, "tree": tree, "remote_head": remote_head, "remote_tree": remote_tree,
        "remote_head_matches": True, "remote_tree_matches": True,
        "anonymous_changed_path_readback": {"path_count": len(paths), "total_bytes": total_bytes,
                                             "all_paths_match_local_bytes": True, "mismatch_count": 0},
        "representative_raw_readback": sorted(representative, key=lambda item: str(item["path"])),
        "credential_material_recorded": False, "result": "pass",
    }
